...
void MPIE::init(component::ConfigLoader * ploader)
{
	EM::init(ploader);
	if (rtype_ == policy::ResultType::Rad)
		readExcEdges(dir_ + '/' + folder_name_ + ".rad");

	dir_ = tool::creatFolder(dir_, folder_name_ + "_MPIE");
	...
	thick = ploader->getThickness_sheet();
...
void MPIE::output()
{
	...
	if (rtype_ == policy::ResultType::Rad)
	{
		LOG(result.getRadiationField(this, &MPIE::getEFiled), "Calculating Radiation");
		TIME_LOG("getRadiation");
		LOG(getCurrentOnFeed(), "Calculating the Current on the feed");
	}
	else
	{
		LOG(result.getBistaticRCS(this, &MPIE::getBiRCS), "Calculating BistaticRCS");
		TIME_LOG("getBistaticRCS");
	}

	//LOG(result.getNearField(this, &MPIE::getNearMPIEld), "Calculating Near field");
	LOG(result.getCurrentDistribution(this, &MPIE::calculateSurfaceCurrent), "Calculating surface current");
}
...
void MPIE::reportInfo(Qostream& strm) const
{
	mesh_ptr_->reportInfo(strm);
	ce_ptr_->reportInfo(strm);
}

Complex MPIE::eZppKernel(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	value_t r;
...
Complex MPIE::eZpmKernel(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	value_t r;
	Complex Ze(0, 0), Zm(0, 0), G(0, 0);
	VectorR3 vgf[3], vgs[3], rou_f, rou_s;
...
Complex MPIE::eZmpKernel(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	value_t r;
	Complex Ze(0, 0), Zm(0, 0), G(0, 0);
	VectorR3 vgf[3], vgs[3], rou_f, rou_s;
...
Complex MPIE::eZmmKernel(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	value_t r;
	Complex Ze(0, 0), Zm(0, 0), G(0, 0);
	VectorR3 vgf[3], vgs[3], rou_f, rou_s;
...
Complex MPIE::eZppSingular(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
...
Complex MPIE::eZpmSingular(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	Complex Ze(0, 0), Ze1(0, 0), Ze2(0, 0), Ze3(0, 0);
	VectorR3 vgf[3], vgs[3], rou_f, rou_s;
...
Complex MPIE::eZmpSingular(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	Complex Ze(0, 0), Ze1(0, 0), Ze2(0, 0), Ze3(0, 0);
	VectorR3 vgf[3], vgs[3], rou_f, rou_s;
...
Complex MPIE::eZmmSingular(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	Complex Ze(0, 0), Ze1(0, 0), Ze2(0, 0), Ze3(0, 0);
	VectorR3 vgf[3], vgs[3], rou_f, rou_s;
...
Complex MPIE::SpatialToSpectrum(VectorR3 &v_src, VectorR3 &v_fld)
{
	Complex R_te, R_q;
	Complex r10_te, r10_tm;
	Complex G_Axx, G_q;
	G_Axx = 1.0f / (J0*2.0f);
	return G_Axx;
}

Complex MPIE::EDMKernel(VectorR3 *vf_p3, VectorR3 *vf_m3, VectorR3 *vs_p3, VectorR3 *vs_m3, value_t &l_fld, value_t &l_src)
{
...
Complex MPIE::eVKernel(VectorR3 *vp3, VectorR3 *vm3, VectorR3 &vp, VectorR3 &vm)
{
...
bool MPIE::readExcEdges(const Qstring & rad_file, Qstring& stateInfo)
{
...
void MPIE::readExcEdges(const Qstring & rad_file)
{
...
value_t MPIE::getBiRCS(const VectorR3 & sca_k) const
{
	int nvp, nvm, fp, fm;
	VectorR3 vp, vm, vp3[3], vm3[3];
	value_t L;
...
void MPIE::getEFiled(component::FieldData * pdata, const VectorR3 & rad_k, const VectorR3 & rad_ev, const VectorR3 & rad_eh) const
{
	int nvp, nvm, fp, fm;
	VectorR3 vp, vm, vp3[3], vm3[3];
	value_t L;
...
void MPIE::calculateSurfaceCurrent(std::vector<component::CurrentData>* currents) const
{
	/*Qcx_vec cur_vec = I;
