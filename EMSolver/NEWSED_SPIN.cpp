...
void NEWSED_SPIN::init(component::ConfigLoader * ploader)
{
	EM::init(ploader);
	...
	dir_ = tool::creatFolder(dir_, folder_name_ + "_NEWSED_SPIN");
	...
	D_ang = ploader->getD_Angle();
	A_ang = ploader->getA_Angle();
	...
	if (isContinuous == 1)
	{
		//ce_ptr_->buildSEDCommonEdge_boundary(mesh_ptr_, D_ang);
...
void NEWSED_SPIN::prepareFSPGF(value_t _Dx, value_t _Dy, int _Array_x, int _Array_y, int _t_sum)
{
...
void NEWSED_SPIN::solve()
{
	...
	{
		for (int j = 0; j < I_sed_eigen.cols(); j++)
		{
...
void NEWSED_SPIN::output()
{
	SEGMENT("Result");
	Result result;
	if (rtype_ == policy::ResultType::Rad)
	{
		LOG(result.getRadiationField(this, &NEWSED_SPIN::getEFiled), "Calculating Radiation");
		TIME_LOG("getRadiation");
	}
	else
	{
		LOG(result.getBistaticRCS(this, &NEWSED_SPIN::getBiRCS), "Calculating BistaticRCS");
		TIME_LOG("getBistaticRCS");
	}
	//LOG(result.getCurrentDistribution(this, &NEWSED_SPIN::calculateSurfaceCurrent), "Calculating surface current");
}

void NEWSED_SPIN::clear()
{
	pool_.clear();
	Z.reset();
	I.reset();
...
void NEWSED_SPIN::reportInfo(Qostream& strm) const
{
	mesh_ptr_->reportInfo(strm);
	ct_ptr_->reportInfo(strm);
}

void NEWSED_SPIN::Task_construct()
{
	for (int f = 0; f < 9; f++)
	{
		for (int s = 0; s < 9; s++)
...
void NEWSED_SPIN::Task_assign(size_t task_num)
{
...
void NEWSED_SPIN::fillSEDZ_MP()
{
	...
	for (int t = 0; t < task_num; t++)
	{
		...
		if (begin < end)
		{
			auto task = std::bind(&NEWSED_SPIN::SEDZ_MP_Kernel, this, begin, end);
			pool_.submit(task);
		}
	}
...
void NEWSED_SPIN::SEDZ_MP_Kernel(int begin, int end)
{
...
void NEWSED_SPIN::fillREDZ_MP()
{
	...
	for (int t = 0; t < task_num; t++)
	{
		...
		if (begin < end)
		{
			auto task = std::bind(&NEWSED_SPIN::REDZ_MP_Kernel, this, begin, end);
			pool_.submit(task);
		}
	}
...
void NEWSED_SPIN::REDZ_MP_Kernel(int begin, int end)
{
	int top, buttom;
	for (int t = begin; t < end; t++)
	{
		int f = SED_task2[t].first;
		int s = SED_task2[t].second;
		...
		CMatrix Z_coup_temp = coupZ_MP(x_m, y_m, x_n, y_n);
		ct_ptr_->getSEDCommonTriangleSize(id_n, top, buttom);
		for (int k_m = 0; k_m < svd_k; k_m++)
		{
			...
			for (int k_n = 0; k_n < svd_k; k_n++)
			{
				...
				CMatrix z_ele = I_m.transpose()*Z_coup_temp*I_n;
...
Complex NEWSED_SPIN::DDZppKernel(VectorR3 * vf4, VectorR3 * vs4, VectorR3 & vf, VectorR3 & vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_p, int &t_s_p, value_t &epsp, value_t &epsm)
{
	value_t r, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex NEWSED_SPIN::DDZpmKernel(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_p, int &t_s_m, value_t &epsp, value_t &epsm)
{
	value_t r, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex NEWSED_SPIN::DDZmpKernel(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_m, int &t_s_p, value_t &epsp, value_t &epsm)
{
	value_t r, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex NEWSED_SPIN::DDZmmKernel(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_m, int &t_s_m, value_t &epsp, value_t &epsm)
{
	value_t r, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex NEWSED_SPIN::DDZppSingular(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_p, int &t_s_p, value_t &epsp, value_t &epsm)
{
...
Complex NEWSED_SPIN::DDZpmSingular(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_p, int &t_s_m, value_t &epsp, value_t &epsm)
{
	value_t r, volume, area, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex NEWSED_SPIN::DDZmpSingular(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_m, int &t_s_p, value_t &epsp, value_t &epsm)
{
	value_t r, volume, area, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex NEWSED_SPIN::DDZmmSingular(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_m, int &t_s_m, value_t &epsp, value_t &epsm)
{
...
Complex NEWSED_SPIN::DDZppKernel_fast(VectorR3 * vf4, VectorR3 * vs4, VectorR3 & vf, VectorR3 & vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_p, int &t_s_p, value_t &epsp, value_t &epsm, KIM& ik)
{
	value_t r, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex NEWSED_SPIN::DDZpmKernel_fast(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_p, int &t_s_m, value_t &epsp, value_t &epsm, KIM& ik)
{
	value_t r, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex NEWSED_SPIN::DDZmpKernel_fast(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_m, int &t_s_p, value_t &epsp, value_t &epsm, KIM& ik)
{
	value_t r, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex NEWSED_SPIN::DDZmmKernel_fast(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_m, int &t_s_m, value_t &epsp, value_t &epsm, KIM& ik)
{
	value_t r, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex NEWSED_SPIN::DDZppSingular_fast(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_p, int &t_s_p, value_t &epsp, value_t &epsm, KIM& ik)
{
	value_t r, volume, area, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex NEWSED_SPIN::DDZpmSingular_fast(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_p, int &t_s_m, value_t &epsp, value_t &epsm, KIM& ik)
{
	value_t r, volume, area, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex NEWSED_SPIN::DDZmpSingular_fast(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_m, int &t_s_p, value_t &epsp, value_t &epsm, KIM& ik)
{
	value_t r, volume, area, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex NEWSED_SPIN::DDZmmSingular_fast(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_m, int &t_s_m, value_t &epsp, value_t &epsm, KIM& ik)
{
	value_t r, volume, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex NEWSED_SPIN::DMZppKernel(VectorR3 * vf4, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs, VectorR3 * vf3, int & _vm_f)
{
...
Complex NEWSED_SPIN::DMZpmKernel(VectorR3 * vf4, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs, VectorR3 * vf3, int & _vm_f)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg4f[4], vg3f[3], vg3s[3], rou_f, rou_s;
	value_t r;
...
Complex NEWSED_SPIN::DMZmpKernel(VectorR3 * vf4, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs, VectorR3 * vf3, int & _vm_f)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg4f[4], vg3f[3], vg3s[3], rou_f, rou_s;
	value_t r;
...
Complex NEWSED_SPIN::DMZmmKernel(VectorR3 * vf4, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs, VectorR3 * vf3, int & _vm_f)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg4f[4], vg3f[3], vg3s[3], rou_f, rou_s;
	value_t r;
...
Complex	NEWSED_SPIN::DMZppSingular(VectorR3 * vf4, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs, VectorR3 * vf3, int & _vm_f)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg4f[4], vg3f[3], vg3s[3], rou_f, rou_s;
	value_t r, area;
...
Complex	NEWSED_SPIN::DMZpmSingular(VectorR3 * vf4, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs, VectorR3 * vf3, int & _vm_f)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg4f[4], vg3f[3], vg3s[3], rou_f, rou_s;
	value_t r, area;
...
Complex NEWSED_SPIN::MDZppkernel(VectorR3 *vf3, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vs3, int &_vm_s, value_t &epsp, value_t &epsm)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
...
Complex NEWSED_SPIN::MDZpmkernel(VectorR3 *vf3, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vs3, int &_vm_s, value_t &epsp, value_t &epsm)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg3f[3], vg3s[3], vg4s[4], rou_f, rou_s;
	value_t r, kp, km;
...
Complex NEWSED_SPIN::MDZmpkernel(VectorR3 *vf3, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vs3, int &_vm_s, value_t &epsp, value_t &epsm)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg3f[3], vg3s[3], vg4s[4], rou_f, rou_s;
	value_t r, kp, km;
...
Complex NEWSED_SPIN::MDZmmkernel(VectorR3 *vf3, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vs3, int &_vm_s, value_t &epsp, value_t &epsm)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg3f[3], vg3s[3], vg4s[4], rou_f, rou_s;
	value_t r, kp, km;
...
Complex NEWSED_SPIN::MDZppSingular(VectorR3 *vf3, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vs3, int &_vm_s, value_t &epsp, value_t &epsm)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg3f[3], vg3s[3], vg4s[4], rou_f, rou_s;
	value_t r, kp, km;
...
Complex NEWSED_SPIN::MDZmpSingular(VectorR3 *vf3, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vs3, int &_vm_s, value_t &epsp, value_t &epsm)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg3f[3], vg3s[3], vg4s[4], rou_f, rou_s;
	value_t r, kp, km;
...
Complex NEWSED_SPIN::MMZppKernel(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	value_t r;
...
Complex NEWSED_SPIN::MMZpmKernel(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	value_t r;
	Complex Ze(0, 0), Zm(0, 0), G(0, 0);
	VectorR3 vgf[3], vgs[3], rou_f, rou_s;
...
Complex NEWSED_SPIN::MMZmpKernel(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	value_t r;
	Complex Ze(0, 0), Zm(0, 0), G(0, 0);
	VectorR3 vgf[3], vgs[3], rou_f, rou_s;
...
Complex NEWSED_SPIN::MMZmmKernel(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	value_t r;
	Complex Ze(0, 0), Zm(0, 0), G(0, 0);
	VectorR3 vgf[3], vgs[3], rou_f, rou_s;
...
Complex NEWSED_SPIN::MMZppSingular(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
...
Complex NEWSED_SPIN::MMZpmSingular(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	/*Complex Ze(0, 0), Ze1(0, 0), Ze2(0, 0);// Ze3(0, 0);
	VectorR3 vgf[3], vgs[3], rou_f, rou_s;
...
Complex NEWSED_SPIN::MMZmpSingular(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	/*Complex Ze(0, 0), Ze1(0, 0), Ze2(0, 0);// Ze3(0, 0);
	VectorR3 vgf[3], vgs[3], rou_f, rou_s;
...
Complex NEWSED_SPIN::MMZmmSingular(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	/*Complex Ze(0, 0), Ze1(0, 0), Ze2(0, 0);// Ze3(0, 0);
	VectorR3 vgf[3], vgs[3], rou_f, rou_s;
...
value_t NEWSED_SPIN::IsSingular(VectorR3 * vs3, VectorR3 & vf)
{
	VectorR3 n = (vs3[1] - vs3[0])*(vs3[2] - vs3[1]);
	VectorR3 ni = n.Normalize();
...
VectorR3 NEWSED_SPIN::IspSingular(VectorR3 * vs3, VectorR3 & vf)
{
	VectorR3 n = (vs3[1] - vs3[0])*(vs3[2] - vs3[1]);
	VectorR3 ni = n.Normalize();
	VectorR3 rou = vf - ni * (ni^vf);
...
value_t NEWSED_SPIN::IvSingular(VectorR3 * vs3, VectorR3 & vf)
{
...
VectorR3 NEWSED_SPIN::IvpSingular(VectorR3 * vs3, VectorR3 & vf)
{
	VectorR3 n = (vs3[1] - vs3[0])*(vs3[2] - vs3[0]);
	VectorR3 ni = n.Normalize();
	VectorR3 rou = vf - ni * (ni^vf);
...
void NEWSED_SPIN::TetToTri(VectorR3 * v4, VectorR3 ** v4_3)
{
	v4_3[0][0] = v4[0];
	v4_3[0][1] = v4[2];
	v4_3[0][2] = v4[1];
...
Complex NEWSED_SPIN::z1(VectorR3 * vf4, VectorR3 * vs4, VectorR3 & vf, VectorR3 & vs, value_t & epsp, value_t & epsm)
{

	return Complex();
}

Complex NEWSED_SPIN::DVKernel(VectorR3 *vp4, VectorR3 *vm4, VectorR3 &vp, VectorR3 &vm, int &_nvm)
{
...
Complex NEWSED_SPIN::MVKernel(VectorR3 *vp3, VectorR3 *vm3, VectorR3 &vp, VectorR3 &vm)
{
...
Complex NEWSED_SPIN::FSPGF(VectorR3 &r, int _t_sum, value_t _Dx, value_t _Dy)
{
...
Complex NEWSED_SPIN::erfz(Complex z)
{
...
void NEWSED_SPIN::KernelIntegral()
{
...
NEWSED_SPIN::KIM NEWSED_SPIN::SED_KernelIntegral(int &sedid_f, int &sedid_s)
{
	Tetrahedron tet_i, tet_j, tet_m, tet_n;
	VectorR3 v_tet_i[4], v_tet_j[4], v_tet_m[4], v_tet_n[4];
	VectorR3 vgi[4], vgj[4], vgm[4], vgn[4];
...
NEWSED_SPIN::KIM NEWSED_SPIN::RED_KernelIntegral(int &xid_f, int &yid_f, int &xid_s, int &yid_s)
{
	Tetrahedron tet_i, tet_j, tet_m, tet_n;
	VectorR3 v_tet_i[4], v_tet_j[4], v_tet_m[4], v_tet_n[4];
	VectorR3 vgi[4], vgj[4], vgm[4], vgn[4];
...
void NEWSED_SPIN::FillZKernel(VectorR3 *vm4, VectorR3 *vn4, int &m, int &n, VectorR3 *v4)
{
	Complex G(0, 0);
	value_t r;
	VectorR3 v4_3[4][3], *Pv4_3[4];
...
void NEWSED_SPIN::FillZKernel_SED_COM(VectorR3 *vm4, VectorR3 *vn4, int &m, int &n, VectorR3 *v4, KIM& ik)
{
	Complex G(0, 0);
	value_t r;
	VectorR3 v4_3[4][3], *Pv4_3[4];
...
void NEWSED_SPIN::FillZKernel_SED_DIFF(VectorR3 *vm4, VectorR3 *vn4, int &m, int &n, VectorR3 *v4, KIM& ik)
{
	Complex G(0, 0);
	value_t r;
	VectorR3 v4_3[4][3], *Pv4_3[4];
...
void NEWSED_SPIN::CalculatePGFgrid()
{
...
Complex NEWSED_SPIN::InterpolarPGF(VectorR3 &r)
{
...
void NEWSED_SPIN::fillZ()
{
	KernelIntegral();
}

void NEWSED_SPIN::fillZDD(int &start_f, int &end_f, int &start_s, int &end_s)
{
...
void NEWSED_SPIN::fillZDM(int &start_f, int &end_f, int &start_s, int &end_s)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
...
void NEWSED_SPIN::fillZMD(int &start_f, int &end_f, int &start_s, int &end_s)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle 
...
void NEWSED_SPIN::fillZMM(int &start_f, int &end_f, int &start_s, int &end_s)
{
	/*int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3], v_fld_plu3[3], v_fld_min3[3];
...
void NEWSED_SPIN::prepareVIE(value_t _eps1, value_t _mu1)
{
	eps1_ = _eps1;
	mu1_ = _mu1;
}

bool NEWSED_SPIN::readExcEdges(const Qstring & rad_file, Qstring& stateInfo)
{
...
void NEWSED_SPIN::readExcEdges(const Qstring & rad_file)
{
...
value_t NEWSED_SPIN::getBiRCS(const VectorR3& sca_k)
{
	int nvp, nvm, fp, fm;
...
value_t NEWSED_SPIN::testRCS(const VectorR3& sca_k)
{
	int nvp, nvm, fp, fm;
	int start_t, end_t, start_e, end_e;
	int sedid, bedge;
...
void NEWSED_SPIN::getEFiled(component::FieldData * pdata, const VectorR3 & rad_k, const VectorR3 & rad_ev, const VectorR3 & rad_eh) const
{
	int nvp, nvm, fp, fm;
...
value_t NEWSED_SPIN::getReflectField(const VectorR3& sca_k) const
{
	int nvp, nvm, fp, fm;
	VectorR3 vp, vm, vp4[4], vm4[4], vp3[3], vm3[3];
	value_t S, L, epsp = 0.0f, epsm = 0.0f;
...
void NEWSED_SPIN::calculateSurfaceCurrent(std::vector<component::CurrentData>* currents) const
{
	Qcx_vec cur_vec = I;
...
void NEWSED_SPIN::fillSEDZDD_MP(int &_f_sed, int &_s_sed, int &_start_tf, int &_end_tf, int &_start_ts, int &_end_ts, int &_start_ef, int &_end_ef, int &_start_es, int &_end_es)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3];
	VectorR3 v_src_plu, v_src_min, v_src_plu4[4], v_src_min4[4], v_src_ct3[3];
...
void NEWSED_SPIN::fillSEDZDD(int &_f_sed, int &_s_sed, int &_start_tf, int &_end_tf, int &_start_ts, int &_end_ts, int &_start_ef, int &_end_ef, int &_start_es, int &_end_es)
{
	//#pragma omp parallel
	{
		int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
...
void NEWSED_SPIN::fillSEDZDM(int &_f_sed, int &_s_sed, int &_start_tf, int &_end_tf, int &_start_ts, int &_end_ts, int &_start_ef, int &_end_ef, int &_start_es, int &_end_es)
{
	//#pragma omp parallel
	{
		int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
...
void NEWSED_SPIN::fillSEDZMD(int &_f_sed, int &_s_sed, int &_start_tf, int &_end_tf, int &_start_ts, int &_end_ts, int &_start_ef, int &_end_ef, int &_start_es, int &_end_es)
{
	//#pragma omp parallel
	{
		int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
...
void NEWSED_SPIN::fillSEDZMM(int &_f_sed, int &_s_sed, int &_start_tf, int &_end_tf, int &_start_ts, int &_end_ts, int &_start_ef, int &_end_ef, int &_start_es, int &_end_es)
{
	//#pragma omp parallel
	{
		int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle
...
void NEWSED_SPIN::fillCOUPZDD(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3];
	VectorR3 v_src_plu, v_src_min, v_src_plu4[4], v_src_min4[4], v_src_ct3[3];
...
void NEWSED_SPIN::fillCOUPZDM(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle 
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3];
...
void NEWSED_SPIN::fillCOUPZMD(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle 
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3], v_fld_plu3[3], v_fld_min3[3];
...
void NEWSED_SPIN::fillCOUPZMM(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n)
{
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu3[3], v_fld_min3[3];
	VectorR3 v_src_plu, v_src_min, v_src_plu3[3], v_src_min3[3];
...
void NEWSED_SPIN::fillCOUPZDD_MP(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n, CMatrix& Z_coup)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3];
	VectorR3 v_src_plu, v_src_min, v_src_plu4[4], v_src_min4[4], v_src_ct3[3];
...
void NEWSED_SPIN::fillCOUPZDM_MP(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n, CMatrix& Z_coup)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle 
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3];
...
void NEWSED_SPIN::fillCOUPZMD_MP(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n, CMatrix& Z_coup)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle 
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3], v_fld_plu3[3], v_fld_min3[3];
...
void NEWSED_SPIN::fillCOUPZMM_MP(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n, CMatrix& Z_coup)
{
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu3[3], v_fld_min3[3];
	VectorR3 v_src_plu, v_src_min, v_src_plu3[3], v_src_min3[3];
...
Complex NEWSED_SPIN::SEDDVKernel(VectorR3 * vp4, VectorR3 * vm4, VectorR3 & vp, VectorR3 & vm, int & _nvm, VectorR3 &inc_k, VectorR3 &inc_e)
{
	VectorR3 vgp[4], vgm[4];
	VectorR3 rou_p, rou_m; //ρ+-
	Complex Ve;
...
Complex NEWSED_SPIN::SEDMVKernel(VectorR3 *vp3, VectorR3 *vm3, VectorR3 &vp, VectorR3 &vm, VectorR3 &inc_k, VectorR3 &inc_e)
{
	VectorR3 vgp[7], vgm[7];
	VectorR3 rou_p, rou_m; //ρ+-
	Complex Ve;
...
void NEWSED_SPIN::REDZKernel(int &_x_m, int &_y_m, int &_x_n, int &_y_n)
{
	int k_f, k_s;
	int bedge_t, sedid_t, bedge_e, sedid_e;
	int start_tf, end_tf, start_ts, end_ts;
...
CMatrix NEWSED_SPIN::coupZ_MP(int &_x_m, int &_y_m, int &_x_n, int &_y_n)
{
	int k_f, k_s;
	int bedge_t, sedid_t, bedge_e, sedid_e;
	int start_tf, end_tf, start_ts, end_ts;
...
void NEWSED_SPIN::REDVKernel(int &_x, int &_y, int &_id)
{
...
void NEWSED_SPIN::fillSVDI()
{
	...
	for (int id = 0; id < 9; id++)
	{
		...
		//I_pw.topRows(end_t - start_t) = J0 * omiga*I_pw.topRows(end_t - start_t);
		/*for (int r = start_t; r < end_t; r++)
		{
		ct_ptr_->getTet_eps(r, eps);
		I_pw.row(r - start_t) = J0 * omiga*(1.0f - 1.0f / eps)*I_pw.row(r - start_t);
		}*/
		...
		//auto& svd_val_m = svd_m.singularValues();
		//auto& svd_mat_m = svd_m.matrixU();
		//auto max_val_m = svd_val_m.maxCoeff();
...
void NEWSED_SPIN::shiftcoordv4(int &sedid, VectorR3 *v4)
{
	int n_x = sedid / 3;
	int n_y = sedid - n_x * 3;
...
void NEWSED_SPIN::shiftcoordv1(int & sedid, VectorR3 & v)
{
	int n_x = sedid / 3;
	int n_y = sedid - n_x * 3;
	VectorR3 shiftvec(n_x*Dx, n_y*Dy, 0.0f);
...
void NEWSED_SPIN::shiftcoordv3(int &sedid, VectorR3 *v3)
{
	int n_x = sedid / 3;
	int n_y = sedid - n_x * 3;
	VectorR3 shiftvec(n_x*Dx, n_y*Dy, 0.0f);
...
void NEWSED_SPIN::shiftcoordv4(int &xid, int &yid, VectorR3 *v4)
{
...
void NEWSED_SPIN::shiftcoordv3(int &xid, int &yid, VectorR3 *v3)
{
	VectorR3 shiftvec(xid*Dx, yid*Dy, 0.0f);
	v3[0] += shiftvec;
	v3[1] += shiftvec;
...
void NEWSED_SPIN::shiftcoordv1(int & xid, int & yid, VectorR3 & v)
{
	VectorR3 shiftvec(xid*Dx, yid*Dy, 0.0f);
	v += shiftvec;
}

void NEWSED_SPIN::shiftbedgecoordv4(int &bedge, VectorR3 *v4)
{
	if (bedge == 1)
	{
...
void NEWSED_SPIN::shiftbedgecoordv3(int &bedge, VectorR3 *v3)
{
	if (bedge == 1)
	{
		VectorR3 shiftvec(Dx, 0.0f, 0.0f);
...
void NEWSED_SPIN::shiftbedgecoordv1(int & bedge, VectorR3 & v)
{
	if (bedge == 1)
	{
		VectorR3 shiftvec(Dx, 0.0f, 0.0f);
...
int NEWSED_SPIN::ArrayToSED(int &_x, int &_y) const
{
	if (_x == 0)
	{
		if (_y == 0)
