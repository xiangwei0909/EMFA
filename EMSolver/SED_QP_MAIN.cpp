...
void SED_QP_MAIN::init(component::ConfigLoader * ploader)
{
	EM::init(ploader);
	ex_num = 0;
	...
	dir_ = tool::creatFolder(dir_, folder_name_ + "_SED_QP_MAIN");
	...
	threshold_edm = 2.0f*cc / incidence_.freq;
	far_threshold = 2.5f*cc / incidence_.freq;
	main_cur_threshold = 0.3f;
	bou_ce1 = 0;
	bou_ce2 = 0;
	if (isContinuous == 1)
	{
...
void SED_QP_MAIN::prepareFSPGF(value_t _Dx, value_t _Dy, int _Array_x, int _Array_y, int _t_sum)
{
...
void SED_QP_MAIN::solve()
{
	...
	LOG(CalculateMainCur(), "Calculate the DM");
	TIME_LOG("Calculate the DM");
...
void SED_QP_MAIN::output()
{
	SEGMENT("Result");
	Result result;
	if (rtype_ == policy::ResultType::Rad)
	{
		LOG(result.getRadiationField(this, &SED_QP_MAIN::getEFiled), "Calculating Radiation");
		TIME_LOG("getRadiation");
		LOG(getCurrentOnFeed(), "Calculating the current on the feed point");
	}
	else
	{
		LOG(result.getBistaticRCS(this, &SED_QP_MAIN::getBiRCS), "Calculating BistaticRCS");
		TIME_LOG("getBistaticRCS");
	}
	LOG(result.getNearField(this, &SED_QP_MAIN::getNearEField), "Calculating Near field");

	LOG(result.getCurrentDistribution(this, &SED_QP_MAIN::calculateSurfaceCurrent), "Calculating surface current");
}

void SED_QP_MAIN::clear()
{
	pool_.clear();
	Z.reset();
	I.reset();
...
void SED_QP_MAIN::reportInfo(Qostream& strm) const
{
	mesh_ptr_->reportInfo(strm);
	ct_ptr_->reportInfo(strm);
}

void SED_QP_MAIN::Task_construct()
{
	for (int f = 0; f < 9; f++)
	{
		for (int s = 0; s < 9; s++)
...
void SED_QP_MAIN::Task_assign(size_t task_num)
{
...
void SED_QP_MAIN::fillSEDZ_MP()
{
	...
	for (int t = 0; t < task_num; t++)
	{
		...
		if (begin < end)
		{
			auto task = std::bind(&SED_QP_MAIN::SEDZ_MP_Kernel, this, begin, end);
			pool_.submit(task);
		}
	}
...
void SED_QP_MAIN::SEDZ_MP_Kernel(int begin, int end)
{
...
void SED_QP_MAIN::fillREDZ_MP()
{
	...
	for (int t = 0; t < task_num; t++)
	{
		...
		if (begin < end)
		{
			auto task = std::bind(&SED_QP_MAIN::REDZ_MP_Kernel, this, begin, end);
			pool_.submit(task);
		}
	}
...
void SED_QP_MAIN::fillREDZ_QP()
{
	...
	for (int t = 0; t < task_num; t++)
	{
		...
		if (begin < end)
		{
			auto task = std::bind(&SED_QP_MAIN::REDZ_QP_Kernel, this, begin, end);
			pool_.submit(task);
		}
	}
	Qcout << "Finish task submit!" << std::endl;
...
void SED_QP_MAIN::REDZ_MP_Kernel(int begin, int end)
{
	for (int t = begin; t < end; t++)
	{
...
void SED_QP_MAIN::REDZ_QP_Kernel(int begin, int end)
{
	for (int t = begin; t < end; t++)
	{
		...
		VectorR3 dis_vec(Dx*(x_m - x_n), Dy * (y_m - y_n), 0.0f);
		//Qcout << setw(5) << f << setw(5) << s << setw(5) << x_m << setw(5) << y_m << setw(5) << x_n << setw(5) << y_n << setw(5) << id_m << setw(5) << id_n << std::endl;
		if ((dis_vec.Norm()) < far_threshold)
		{
			CMatrix Z_coup_all = coupZ_MP(x_m, y_m, x_n, y_n);
			for (int k_m = 0; k_m < svd_k; k_m++)
			{
		...
		else
		{
			...
			for (int k_m = 0; k_m < svd_k; k_m++)
			{
				CVector I_m = I_far_eigen[id_m].col(k_m);
				int r = (x_m*Array_y + y_m) * svd_k + k_m;

				for (int k_n = 0; k_n < svd_k; k_n++)
				{
					CVector I_n = I_far_eigen[id_n].col(k_n);
					int c = (x_n*Array_y + y_n) * svd_k + k_n;
...
Complex SED_QP_MAIN::DDZppKernel(VectorR3 * vf4, VectorR3 * vs4, VectorR3 & vf, VectorR3 & vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_p, int &t_s_p, value_t &epsp, value_t &epsm)
{
	value_t r, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex SED_QP_MAIN::DDZpmKernel(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_p, int &t_s_m, value_t &epsp, value_t &epsm)
{
	value_t r, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex SED_QP_MAIN::DDZmpKernel(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_m, int &t_s_p, value_t &epsp, value_t &epsm)
{
	value_t r, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex SED_QP_MAIN::DDZmmKernel(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_m, int &t_s_m, value_t &epsp, value_t &epsm)
{
	value_t r, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex SED_QP_MAIN::DDZppSingular(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_p, int &t_s_p, value_t &epsp, value_t &epsm)
{
...
Complex SED_QP_MAIN::DDZpmSingular(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_p, int &t_s_m, value_t &epsp, value_t &epsm)
{
	value_t r, volume, area, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex SED_QP_MAIN::DDZmpSingular(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_m, int &t_s_p, value_t &epsp, value_t &epsm)
{
	value_t r, volume, area, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex SED_QP_MAIN::DDZmmSingular(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_m, int &t_s_m, value_t &epsp, value_t &epsm)
{
...
Complex SED_QP_MAIN::DDZppKernel_fast(VectorR3 * vf4, VectorR3 * vs4, VectorR3 & vf, VectorR3 & vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_p, int &t_s_p, value_t &epsp, value_t &epsm, KIM& ik)
{
	value_t r, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex SED_QP_MAIN::DDZpmKernel_fast(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_p, int &t_s_m, value_t &epsp, value_t &epsm, KIM& ik)
{
	value_t r, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex SED_QP_MAIN::DDZmpKernel_fast(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_m, int &t_s_p, value_t &epsp, value_t &epsm, KIM& ik)
{
	value_t r, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex SED_QP_MAIN::DDZmmKernel_fast(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_m, int &t_s_m, value_t &epsp, value_t &epsm, KIM& ik)
{
	value_t r, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex SED_QP_MAIN::DDZppSingular_fast(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_p, int &t_s_p, value_t &epsp, value_t &epsm, KIM& ik)
{
	value_t r, volume, area, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex SED_QP_MAIN::DDZpmSingular_fast(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_p, int &t_s_m, value_t &epsp, value_t &epsm, KIM& ik)
{
	value_t r, volume, area, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex SED_QP_MAIN::DDZmpSingular_fast(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_m, int &t_s_p, value_t &epsp, value_t &epsm, KIM& ik)
{
	value_t r, volume, area, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex SED_QP_MAIN::DDZmmSingular_fast(VectorR3 *vf4, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vf3, VectorR3 *vs3, int &_vm_f, int &_vm_s, int &t_f_m, int &t_s_m, value_t &epsp, value_t &epsm, KIM& ik)
{
	value_t r, volume, kp, km;
	kp = (epsp - 1.0f) / epsp;
	km = (epsm - 1.0f) / epsm;
...
Complex SED_QP_MAIN::DMZppKernel(VectorR3 * vf4, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs, VectorR3 * vf3, int & _vm_f)
{
...
Complex SED_QP_MAIN::DMZpmKernel(VectorR3 * vf4, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs, VectorR3 * vf3, int & _vm_f)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg4f[4], vg3f[3], vg3s[3], rou_f, rou_s;
	value_t r;
...
Complex SED_QP_MAIN::DMZmpKernel(VectorR3 * vf4, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs, VectorR3 * vf3, int & _vm_f)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg4f[4], vg3f[3], vg3s[3], rou_f, rou_s;
	value_t r;
...
Complex SED_QP_MAIN::DMZmmKernel(VectorR3 * vf4, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs, VectorR3 * vf3, int & _vm_f)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg4f[4], vg3f[3], vg3s[3], rou_f, rou_s;
	value_t r;
...
Complex	SED_QP_MAIN::DMZppSingular(VectorR3 * vf4, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs, VectorR3 * vf3, int & _vm_f)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg4f[4], vg3f[3], vg3s[3], rou_f, rou_s;
	value_t r, area;
...
Complex	SED_QP_MAIN::DMZpmSingular(VectorR3 * vf4, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs, VectorR3 * vf3, int & _vm_f)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg4f[4], vg3f[3], vg3s[3], rou_f, rou_s;
	value_t r, area;
...
Complex SED_QP_MAIN::MDZppkernel(VectorR3 *vf3, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vs3, int &_vm_s, value_t &epsp, value_t &epsm)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
...
Complex SED_QP_MAIN::MDZpmkernel(VectorR3 *vf3, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vs3, int &_vm_s, value_t &epsp, value_t &epsm)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg3f[3], vg3s[3], vg4s[4], rou_f, rou_s;
	value_t r, kp, km;
...
Complex SED_QP_MAIN::MDZmpkernel(VectorR3 *vf3, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vs3, int &_vm_s, value_t &epsp, value_t &epsm)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg3f[3], vg3s[3], vg4s[4], rou_f, rou_s;
	value_t r, kp, km;
...
Complex SED_QP_MAIN::MDZmmkernel(VectorR3 *vf3, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vs3, int &_vm_s, value_t &epsp, value_t &epsm)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg3f[3], vg3s[3], vg4s[4], rou_f, rou_s;
	value_t r, kp, km;
...
Complex SED_QP_MAIN::MDZppSingular(VectorR3 *vf3, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vs3, int &_vm_s, value_t &epsp, value_t &epsm)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg3f[3], vg3s[3], vg4s[4], rou_f, rou_s;
	value_t r, kp, km;
...
Complex SED_QP_MAIN::MDZmpSingular(VectorR3 *vf3, VectorR3 *vs4, VectorR3 &vf, VectorR3 &vs, VectorR3 *vs3, int &_vm_s, value_t &epsp, value_t &epsm)
{
	Complex Z(0.0f, 0.0f), Z1(0.0f, 0.0f), Z2(0.0f, 0.0f), Z3(0.0f, 0.0f), G(0.0f, 0.0f);
	VectorR3 vg3f[3], vg3s[3], vg4s[4], rou_f, rou_s;
	value_t r, kp, km;
...
Complex SED_QP_MAIN::MMZppKernel(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	value_t r;
...
Complex SED_QP_MAIN::MMZpmKernel(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	value_t r;
	Complex Ze(0, 0), Zm(0, 0), G(0, 0);
	VectorR3 vgf[3], vgs[3], rou_f, rou_s;
...
Complex SED_QP_MAIN::MMZmpKernel(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	value_t r;
	Complex Ze(0, 0), Zm(0, 0), G(0, 0);
	VectorR3 vgf[3], vgs[3], rou_f, rou_s;
...
Complex SED_QP_MAIN::MMZmmKernel(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	value_t r;
	Complex Ze(0, 0), Zm(0, 0), G(0, 0);
	VectorR3 vgf[3], vgs[3], rou_f, rou_s;
...
Complex SED_QP_MAIN::MMZppSingular(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
...
Complex SED_QP_MAIN::MMZpmSingular(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	/*Complex Ze(0, 0), Ze1(0, 0), Ze2(0, 0);// Ze3(0, 0);
	VectorR3 vgf[3], vgs[3], rou_f, rou_s;
...
Complex SED_QP_MAIN::MMZmpSingular(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	/*Complex Ze(0, 0), Ze1(0, 0), Ze2(0, 0);// Ze3(0, 0);
	VectorR3 vgf[3], vgs[3], rou_f, rou_s;
...
Complex SED_QP_MAIN::MMZmmSingular(VectorR3 * vf3, VectorR3 * vs3, VectorR3 & vf, VectorR3 & vs)
{
	/*Complex Ze(0, 0), Ze1(0, 0), Ze2(0, 0);// Ze3(0, 0);
	VectorR3 vgf[3], vgs[3], rou_f, rou_s;
...
Complex SED_QP_MAIN::MMEDMKernel(VectorR3 *vf_p3, VectorR3 *vf_m3, VectorR3 *vs_p3, VectorR3 *vs_m3, value_t &l_fld, value_t &l_src)
{
...
value_t SED_QP_MAIN::IsSingular(VectorR3 * vs3, VectorR3 & vf)
{
	VectorR3 n = (vs3[1] - vs3[0])*(vs3[2] - vs3[1]);
	VectorR3 ni = n.Normalize();
...
VectorR3 SED_QP_MAIN::IspSingular(VectorR3 * vs3, VectorR3 & vf)
{
	VectorR3 n = (vs3[1] - vs3[0])*(vs3[2] - vs3[1]);
	VectorR3 ni = n.Normalize();
	VectorR3 rou = vf - ni * (ni^vf);
...
value_t SED_QP_MAIN::IvSingular(VectorR3 * vs3, VectorR3 & vf)
{
...
VectorR3 SED_QP_MAIN::IvpSingular(VectorR3 * vs3, VectorR3 & vf)
{
	VectorR3 n = (vs3[1] - vs3[0])*(vs3[2] - vs3[0]);
	VectorR3 ni = n.Normalize();
	VectorR3 rou = vf - ni * (ni^vf);
...
void SED_QP_MAIN::TetToTri(VectorR3 * v4, VectorR3 ** v4_3)
{
	v4_3[0][0] = v4[0];
	v4_3[0][1] = v4[2];
	v4_3[0][2] = v4[1];
...
Complex SED_QP_MAIN::z1(VectorR3 * vf4, VectorR3 * vs4, VectorR3 & vf, VectorR3 & vs, value_t & epsp, value_t & epsm)
{

	return Complex();
}

Complex SED_QP_MAIN::DVKernel(VectorR3 *vp4, VectorR3 *vm4, VectorR3 &vp, VectorR3 &vm, int &_nvm)
{
...
Complex SED_QP_MAIN::MVKernel(VectorR3 *vp3, VectorR3 *vm3, VectorR3 &vp, VectorR3 &vm)
{
...
Complex SED_QP_MAIN::FSPGF(VectorR3 &r, int _t_sum, value_t _Dx, value_t _Dy)
{
...
Complex SED_QP_MAIN::erfz(Complex z)
{
...
void SED_QP_MAIN::KernelIntegral()
{
...
SED_QP_MAIN::KIM SED_QP_MAIN::SED_KernelIntegral(int &sedid_f, int &sedid_s)
{
	Tetrahedron tet_i, tet_j, tet_m, tet_n;
	VectorR3 v_tet_i[4], v_tet_j[4], v_tet_m[4], v_tet_n[4];
	VectorR3 vgi[4], vgj[4], vgm[4], vgn[4];
...
SED_QP_MAIN::KIM SED_QP_MAIN::RED_KernelIntegral(int &xid_f, int &yid_f, int &xid_s, int &yid_s)
{
	Tetrahedron tet_i, tet_j, tet_m, tet_n;
	VectorR3 v_tet_i[4], v_tet_j[4], v_tet_m[4], v_tet_n[4];
	VectorR3 vgi[4], vgj[4], vgm[4], vgn[4];
...
void SED_QP_MAIN::FillZKernel(VectorR3 *vm4, VectorR3 *vn4, int &m, int &n, VectorR3 *v4)
{
	Complex G(0, 0);
	value_t r;
	VectorR3 v4_3[4][3], *Pv4_3[4];
...
void SED_QP_MAIN::FillZKernel_SED_COM(VectorR3 *vm4, VectorR3 *vn4, int &m, int &n, VectorR3 *v4, KIM& ik)
{
	Complex G(0, 0);
	value_t r;
	VectorR3 v4_3[4][3], *Pv4_3[4];
...
void SED_QP_MAIN::FillZKernel_SED_DIFF(VectorR3 *vm4, VectorR3 *vn4, int &m, int &n, VectorR3 *v4, KIM& ik)
{
	Complex G(0, 0);
	value_t r;
	VectorR3 v4_3[4][3], *Pv4_3[4];
...
void SED_QP_MAIN::CalculatePGFgrid()
{
...
Complex SED_QP_MAIN::InterpolarPGF(VectorR3 &r)
{
...
void SED_QP_MAIN::fillZ()
{
	KernelIntegral();
}

void SED_QP_MAIN::fillZDD(int &start_f, int &end_f, int &start_s, int &end_s)
{
...
void SED_QP_MAIN::fillZDM(int &start_f, int &end_f, int &start_s, int &end_s)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
...
void SED_QP_MAIN::fillZMD(int &start_f, int &end_f, int &start_s, int &end_s)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle 
...
void SED_QP_MAIN::fillZMM(int &start_f, int &end_f, int &start_s, int &end_s)
{
	/*int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3], v_fld_plu3[3], v_fld_min3[3];
...
void SED_QP_MAIN::prepareVIE(value_t _eps1, value_t _mu1)
{
	eps1_ = _eps1;
	mu1_ = _mu1;
}

bool SED_QP_MAIN::readExcEdges(const Qstring & rad_file, Qstring& stateInfo)
{
...
void SED_QP_MAIN::readExcEdges(const Qstring & rad_file)
{
...
value_t SED_QP_MAIN::getBiRCS(const VectorR3& sca_k)
{
	int nvp, nvm, fp, fm;
	int nv1, nv2;
...
value_t SED_QP_MAIN::testRCS(const VectorR3& sca_k)
{
	int nvp, nvm, fp, fm;
	int start_t, end_t, start_e, end_e;
	int sedid, bedge;
...
void SED_QP_MAIN::getEFiled(component::FieldData * pdata, const VectorR3 & rad_k, const VectorR3 & rad_ev, const VectorR3 & rad_eh) const
{
	int nvp, nvm, fp, fm;
	int nv1, nv2;
	int start_t, end_t, start_e, end_e;
...
void SED_QP_MAIN::getCurrentOnFeed()
{
	int nv1, nv2;
	int nvp, nvm, fp, fm;
	int start_t, end_t, start_e, end_e;
...
void SED_QP_MAIN::getNearEField(std::vector<component::NearFieldData>* data) const
{
...
void SED_QP_MAIN::NEFkernel_vol(VectorR3& ob, int& unk, VectorC3& e_vol) const
{
...
void SED_QP_MAIN::NEFkernel_surf(VectorR3& ob, int& unk, VectorC3& e_surf) const
{
...
void SED_QP_MAIN::NEFkernel_surf(VectorR3& ob, int& _x, int& _y, int& unk, component::VectorC3& e_surf) const
{
...
value_t SED_QP_MAIN::getReflectField(const VectorR3& sca_k) const
{
	int nvp, nvm, fp, fm;
	VectorR3 vp, vm, vp4[4], vm4[4], vp3[3], vm3[3];
	value_t S, L, epsp = 0.0f, epsm = 0.0f;
...
void SED_QP_MAIN::calculateSurfaceCurrent(std::vector<component::CurrentData>* currents) const
{
	...
	value_t scalex = scale_x;
	value_t scaley = scale_y;
	value_t rotatex = rotate_x;
	value_t rotatey = rotate_y;
	...
	for (int x = 0; x < Array_x; x++)
	{
		for (int y = 0; y < Array_y; y++)
		{
			...
			for (int t = 0; t < tri_num; t++)
			{
				...
				scalecoordv3(x, y, v, scalex, scaley, rotatex, rotatey);
				...
				for (int k = 0; k < 3; k++)//遍历该三角形的三个顶点
				{
					...
					for (int e = 0; e < ce.size(); e++)//遍历该三角形的所有公共边，计算公共边对该顶点的贡献
					{
						...
						if (t == fp)
						{
							...
							scalecoordv1(x, y, vx, scalex, scaley, rotatex, rotatey);
						}
						else
						{
						...
						if (n[k] == nv1)
						{
							...
							scalecoordv1(x, y, v1, scalex, scaley, rotatex, rotatey);
							cur_node_vec += sign * I_vec(ce[e] - start)*(v1 - vx)*len / (2.0f * area);
						}
						else if (n[k] == nv2)
						{
							VectorR3 v2 = mesh_ptr_->getVertex(n[k]);
							scalecoordv1(x, y, v2, scalex, scaley, rotatex, rotatey);
				...
				for (int e = 0; e < ce.size(); e++)//计算所有公共边对中心点的贡献
				{
					...
					if (t == fp)
					{
						...
						scalecoordv1(x, y, vp, scalex, scaley, rotatex, rotatey);
						cur_cen_vec += I_vec(ce[e] - start)*(v_cen - vp)*len / (2.0f * area);
					}
					else
					{
						VectorR3 vm = mesh_ptr_->getVertex(nvm);
						scalecoordv1(x, y, vm, scalex, scaley, rotatex, rotatey);
			...
			for (int t = 0; t < tri_num; t++)
			{
				...
				//value_t scale = scale_x;
				shiftcoordv3(x, y, v, scalex, scaley, rotatex, rotatey);
...
void SED_QP_MAIN::readFEKOcurrent()
{
	int tri_num;
...
void SED_QP_MAIN::fillSEDZDD_MP(int &_f_sed, int &_s_sed, int &_start_tf, int &_end_tf, int &_start_ts, int &_end_ts, int &_start_ef, int &_end_ef, int &_start_es, int &_end_es)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3];
	VectorR3 v_src_plu, v_src_min, v_src_plu4[4], v_src_min4[4], v_src_ct3[3];
...
void SED_QP_MAIN::fillSEDZDD(int &_f_sed, int &_s_sed, int &_start_tf, int &_end_tf, int &_start_ts, int &_end_ts, int &_start_ef, int &_end_ef, int &_start_es, int &_end_es)
{
	//#pragma omp parallel
	{
		int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
...
void SED_QP_MAIN::fillSEDZDM(int &_f_sed, int &_s_sed, int &_start_tf, int &_end_tf, int &_start_ts, int &_end_ts, int &_start_ef, int &_end_ef, int &_start_es, int &_end_es)
{
	//#pragma omp parallel
	{
		int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
...
void SED_QP_MAIN::fillSEDZMD(int &_f_sed, int &_s_sed, int &_start_tf, int &_end_tf, int &_start_ts, int &_end_ts, int &_start_ef, int &_end_ef, int &_start_es, int &_end_es)
{
	//#pragma omp parallel
	{
		int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
...
void SED_QP_MAIN::fillSEDZMM(int &_f_sed, int &_s_sed, int &_start_tf, int &_end_tf, int &_start_ts, int &_end_ts, int &_start_ef, int &_end_ef, int &_start_es, int &_end_es)
{
	//#pragma omp parallel
	{
		int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle
...
void SED_QP_MAIN::fillCOUPZDD(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3];
	VectorR3 v_src_plu, v_src_min, v_src_plu4[4], v_src_min4[4], v_src_ct3[3];
...
void SED_QP_MAIN::fillCOUPZDM(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle 
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3];
...
void SED_QP_MAIN::fillCOUPZMD(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle 
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3], v_fld_plu3[3], v_fld_min3[3];
...
void SED_QP_MAIN::fillCOUPZMM(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n)
{
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu3[3], v_fld_min3[3];
	VectorR3 v_src_plu, v_src_min, v_src_plu3[3], v_src_min3[3];
...
void SED_QP_MAIN::fillCOUPZDD_MP(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n, CMatrix& Z_coup)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3];
	VectorR3 v_src_plu, v_src_min, v_src_plu4[4], v_src_min4[4], v_src_ct3[3];
...
void SED_QP_MAIN::fillCOUPZDM_MP(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n, CMatrix& Z_coup)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle 
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3];
...
void SED_QP_MAIN::fillCOUPZMD_MP(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n, CMatrix& Z_coup)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle 
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3], v_fld_plu3[3], v_fld_min3[3];
...
void SED_QP_MAIN::fillCOUPZMM_MP(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n, CMatrix& Z_coup)
{
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu3[3], v_fld_min3[3];
	VectorR3 v_src_plu, v_src_min, v_src_plu3[3], v_src_min3[3];
	...
	//coef = (J0*k_*Z0) / PI4;
	//#pragma omp parallel for
	VectorR3 dis_vec(Dx*(_x_m - _x_n), Dy * (_y_m - _y_n), 0.0f);
	if ((dis_vec.Norm()) < far_threshold)
	{
		Z_coup.resize(end_ef - start_ef, end_es - start_es);
		for (int f = start_ef; f < end_ef; f++)
	...
	{
		Z_coup.resize(MainRwg[_k_m].size(), MainRwg[_k_n].size());
		int f_cur = 0;
		for (std::set<int>::iterator f = MainRwg[_k_m].begin(); f != MainRwg[_k_m].end(); f++, f_cur++)
		{
			//bar_perc4(f - start_ef + 1, end_ef - start_ef);
			ce_ptr_->getCommonEdge(*f, vp, vm, f_fld_plu, f_fld_min, l_fld, sedid_f, bedge_f);
			...
			int s_cur = 0;
			for (std::set<int>::iterator s = MainRwg[_k_n].begin(); s != MainRwg[_k_n].end(); s++, s_cur++)
			{
				Complex Zpp(0, 0), Zpm(0, 0), Zmp(0, 0), Zmm(0, 0);
				ce_ptr_->getCommonEdge(*s, vp, vm, f_src_plu, f_src_min, l_src, sedid_s, bedge_s);
				...
				if ((v_fld_cen - v_src_cen).Norm() > threshold_edm)
				{
					Z_coup(f_cur , s_cur) = MMEDMKernel(v_fld_plu3, v_fld_min3, v_src_plu3, v_src_min3, l_fld, l_src);
				}
				else
				{
					//field face+ <-->  source face+
					...
					Z_coup(f_cur, s_cur) = coef * l_fld * l_src * (Zpp + Zpm + Zmp + Zmm);
				}
			}
		}
	}
...
void SED_QP_MAIN::fillCOUPZDD_MP_C(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n, CMatrix& Z_coup)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3];
	VectorR3 v_src_plu, v_src_min, v_src_plu4[4], v_src_min4[4], v_src_ct3[3];
...
void SED_QP_MAIN::fillCOUPZDM_MP_C(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n, CMatrix& Z_coup)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle 
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3];
...
void SED_QP_MAIN::fillCOUPZMD_MP_C(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n, CMatrix& Z_coup)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle 
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3], v_fld_plu3[3], v_fld_min3[3];
...
void SED_QP_MAIN::fillCOUPZMM_MP_C(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n, CMatrix& Z_coup)
{
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu3[3], v_fld_min3[3];
	VectorR3 v_src_plu, v_src_min, v_src_plu3[3], v_src_min3[3];
...
void SED_QP_MAIN::fillCOUPZDD_MP_B(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n, CMatrix& Z_coup)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3];
	VectorR3 v_src_plu, v_src_min, v_src_plu4[4], v_src_min4[4], v_src_ct3[3];
...
void SED_QP_MAIN::fillCOUPZDM_MP_B(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n, CMatrix& Z_coup)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle 
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3];
...
void SED_QP_MAIN::fillCOUPZMD_MP_B(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n, CMatrix& Z_coup)
{
	int t_fld_plu, t_fld_min, t_src_plu, t_src_min; //tetrahedron +- of source and field triangle
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle 
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu4[4], v_fld_min4[4], v_fld_ct3[3], v_fld_plu3[3], v_fld_min3[3];
...
void SED_QP_MAIN::fillCOUPZMM_MP_B(int &_x_m, int &_y_m, int &_x_n, int &_y_n, int &_k_m, int &_k_n, CMatrix& Z_coup)
{
	int f_fld_plu, f_fld_min, f_src_plu, f_src_min; //face +- of source and field triangle
	VectorR3 v_fld_plu, v_fld_min, v_fld_plu3[3], v_fld_min3[3];
	VectorR3 v_src_plu, v_src_min, v_src_plu3[3], v_src_min3[3];
...
Complex SED_QP_MAIN::SEDDVKernel(VectorR3 * vp4, VectorR3 * vm4, VectorR3 & vp, VectorR3 & vm, int & _nvm, VectorR3 &inc_k, VectorR3 &inc_e)
{
	VectorR3 vgp[4], vgm[4];
	VectorR3 rou_p, rou_m; //ρ+-
	Complex Ve;
...
Complex SED_QP_MAIN::SEDMVKernel(VectorR3 *vp3, VectorR3 *vm3, VectorR3 &vp, VectorR3 &vm, VectorR3 &inc_k, VectorR3 &inc_e)
{
	VectorR3 vgp[7], vgm[7];
	VectorR3 rou_p, rou_m; //ρ+-
	Complex Ve;
...
void SED_QP_MAIN::fillREDV_MP()
{
	...
	for (int t = 0; t < task_num; t++)
	{
		...
		if (begin < end)
		{
			auto task = std::bind(&SED_QP_MAIN::REDV_MP_Kernel, this, begin, end);
			pool_.submit(task);
		}
	}
...
void SED_QP_MAIN::REDV_MP_Kernel(int begin, int end)
{
...
void SED_QP_MAIN::REDZKernel(int &_x_m, int &_y_m, int &_x_n, int &_y_n)
{
	int k_f, k_s;
	int bedge_t, sedid_t, bedge_e, sedid_e;
	int start_tf, end_tf, start_ts, end_ts;
...
CMatrix SED_QP_MAIN::coupZ_MP(int &_x_m, int &_y_m, int &_x_n, int &_y_n)
{
	int k_f, k_s;
	int bedge_t, sedid_t, bedge_e, sedid_e;
	int start_tf, end_tf, start_ts, end_ts;
...
CMatrix SED_QP_MAIN::coupZ_MP_COM(int &_x_m, int &_y_m, int &_x_n, int &_y_n)
{
	int k_f, k_s;
...
CVector SED_QP_MAIN::REDVKernel(int &_x, int &_y, int &_id)
{
...
void SED_QP_MAIN::REDVKernel_rad(int &_x, int &_y, int &_id)
{
...
void SED_QP_MAIN::fillSVDI()
{
	...
	//**********如果只有MPW两句都注释***********//
	svd_k = I_Jcm_eigen[0].cols();//only CMs
	//svd_k += 10;//CMs+MPW
...
void SED_QP_MAIN::CalculateFarCurModes()
{
	int start_e, end_e;
	int vp, vm, f_plu, f_min;

	value_t len;
	Triangle tri_plu,tri_min;
	VectorR3 v_plu, v_min, v_plu3[3], v_min3[3];

	

	for (int blk = 0; blk < 9; blk++)
	{
		ce_ptr_->getSEDCommonEdgeSize(blk, start_e, end_e);
		int rwg_num = I_svd_eigen[blk].rows();
		auto& jcm = I_svd_eigen[blk];
		auto& jcm_norm = jcm.cwiseAbs2();
		value_t maxvalue = jcm_norm.maxCoeff();

		Qcout <<"The max value of current:"<< rwg_num << std::endl;
		std::set<int> blkmainrwg;
		//system("pause");
		for (int k = 0; k < svd_k; k++)
		{
			//auto& jcm = I_svd_eigen[blk].col(k);
			for (int i = 0; i < rwg_num; i++)
			{
				value_t ele = jcm_norm(i, svd_k-1-k);
				if ((ele / maxvalue) < main_cur_threshold)
					continue;
				else
					blkmainrwg.insert(i);
			}
		}
		MainRwg.push_back(blkmainrwg);
		Qcout << "the number of main rwg:" <<'\n'<< blkmainrwg.size() << std::endl;
	}
	Qcout << "Finish Calculate DM!" << std::endl;

	for (int blk = 0; blk < 9; blk++)
	{
		int row = MainRwg[blk].size();
		CMatrix far_current(row, svd_k);
		int cur = 0;
		for (std::set<int>::iterator it = MainRwg[blk].begin(); it != MainRwg[blk].end(); it++,cur++)
		{
			far_current.row(cur) = I_svd_eigen[blk].row(*it);
		}
		I_far_eigen.push_back(far_current);
	}
	system("pause");

}

void SED_QP_MAIN::CalculateMainCur()
{
	int start_e, end_e;
	int vp, vm, f_plu, f_min;
	...
	std::vector<VaVector> Tri_cencur_mag;//单元在不同模式下网格中心的电流幅度
	int tri_num = mesh_ptr_->getTriangleNum();

	for (int blk = 0; blk < 9; blk++)
	{
		std::set<int> blkmaince;
		for (int k = 0; k < svd_k; k++)
		{
			std::vector<std::vector<VectorC3>> Tri_centerCur;//当前模式下，各个基函数对三角形中心的电流贡献
			std::vector<std::vector<int>> Tri_SCE;

			Tri_centerCur.resize(tri_num);
			Tri_SCE.resize(tri_num);

			int rwg_num = I_svd_eigen[blk].rows();

			for (int i = 0; i < rwg_num; i++)//遍历基函数，计算电流贡献
			{
				ce_ptr_->getCommonEdge(i, vp, vm, f_plu, f_min, len);

				Triangle tri_plu = mesh_ptr_->getTriangleRef(f_plu);
				Triangle tri_min = mesh_ptr_->getTriangleRef(f_min);

				VectorR3 cen_plu = tri_plu.getCenter();
				VectorR3 cen_min = tri_min.getCenter();

				tri_plu.getVertex(v_plu3[0], v_plu3[1], v_plu3[2]);
				tri_min.getVertex(v_min3[0], v_min3[1], v_min3[2]);

				v_plu = mesh_ptr_->getVertex(vp);
				v_min = mesh_ptr_->getVertex(vm);

				auto& cur_plu = Tri_centerCur[f_plu];
				auto& cur_min = Tri_centerCur[f_min];

				VectorC3 cur_cen_plu = (I_svd_eigen[blk].col(k))(i)*(cen_plu - v_plu)*len*(2.0f*Area(v_plu3[0], v_plu3[1], v_plu3[2]));
				VectorC3 cur_cen_min = (I_svd_eigen[blk].col(k))(i)*(v_min - cen_min)*len*(2.0f*Area(v_min3[0], v_min3[1], v_min3[2]));

				cur_plu.push_back(cur_cen_plu);
				cur_min.push_back(cur_cen_min);
				 
				Tri_SCE[f_plu].push_back(i);
				Tri_SCE[f_min].push_back(i);
			}

			VaVector blk_tri_cencur(tri_num);
			for (int tri = 0; tri < tri_num; tri++)//将前一个循环计算的电流贡献叠加为幅度值
			{
				auto& cen_cur = Tri_centerCur[tri];
				VectorC3 cur_vec;
				for (int i = 0; i < cen_cur.size(); i++)
					cur_vec += cen_cur[i];
				blk_tri_cencur(tri) = cur_vec.norm();
			}

			value_t cur_max = blk_tri_cencur.maxCoeff();
			for (int tri = 0; tri < tri_num; tri++)
			{
				if ((blk_tri_cencur(tri) / cur_max) < main_cur_threshold)
					continue;
				else
				{
					auto& trice = Tri_SCE[tri];
					for (auto& ce : trice)
						blkmaince.insert(ce);
				}
			}
		}
		MainRwg.push_back(blkmaince);
		Qcout << "The number of the main current is : " << MainRwg[blk].size() << std::endl;
	}
	Qcout << "Finish Calculate DM!" << std::endl;
...
void SED_QP_MAIN::shiftcoordv4(int &sedid, VectorR3 *v4) const
{
	int n_x = sedid / 3;
	int n_y = sedid - n_x * 3;
...
void SED_QP_MAIN::shiftcoordv1(int & sedid, VectorR3 & v) const
{
	int n_x = sedid / 3;
	int n_y = sedid - n_x * 3;
	VectorR3 shiftvec(n_x*Dx, n_y*Dy, 0.0f);
...
void SED_QP_MAIN::shiftcoordv3(int &sedid, VectorR3 *v3) const
{
	int n_x = sedid / 3;
	int n_y = sedid - n_x * 3;
	VectorR3 shiftvec(n_x*Dx, n_y*Dy, 0.0f);
...
void SED_QP_MAIN::shiftcoordv4(int &xid, int &yid, VectorR3 *v4) const
{
...
void SED_QP_MAIN::shiftcoordv3(int &xid, int &yid, VectorR3 *v3) const
{
	VectorR3 shiftvec(xid*Dx, yid*Dy, 0.0f);
	v3[0] += shiftvec;
	v3[1] += shiftvec;
...
void SED_QP_MAIN::shiftcoordv1(int & xid, int & yid, VectorR3 & v) const
{
	VectorR3 shiftvec(xid*Dx, yid*Dy, 0.0f);
	v += shiftvec;
}

void SED_QP_MAIN::shiftbedgecoordv4(int &bedge, VectorR3 *v4) const
{
	if (bedge == 1)
	{
...
void SED_QP_MAIN::shiftbedgecoordv3(int &bedge, VectorR3 *v3) const
{
	if (bedge == 1)
	{
		VectorR3 shiftvec(Dx, 0.0f, 0.0f);
...
void SED_QP_MAIN::shiftbedgecoordv1(int & bedge, VectorR3 & v) const
{
	if (bedge == 1)
	{
		VectorR3 shiftvec(Dx, 0.0f, 0.0f);
...
void SED_QP_MAIN::shiftcoordv4(int &sedid, VectorR3 *v4, value_t &scale) const
{
	int n_x = sedid / 3;
	int n_y = sedid - n_x * 3;
...
void SED_QP_MAIN::shiftcoordv1(int & sedid, VectorR3 & v, value_t &scale) const
{
	int n_x = sedid / 3;
	int n_y = sedid - n_x * 3;
	int offset = 0;
...
void SED_QP_MAIN::shiftcoordv3(int &sedid, VectorR3 *v3, value_t &scale) const
{
	int n_x = sedid / 3;
	int n_y = sedid - n_x * 3;
	int offset = 0;
...
void SED_QP_MAIN::shiftcoordv1(int & sedid, VectorR3 & v, value_t &scale_x, value_t &scale_y, value_t &rotate_x, value_t &rotate_y) const
{
	int n_x = sedid / 3;
	int n_y = sedid - n_x * 3;
	int offset = (Array_x / 2) - 1;
...
void SED_QP_MAIN::shiftcoordv3(int &sedid, VectorR3 *v3, value_t &scale_x, value_t &scale_y, value_t &rotate_x, value_t &rotate_y) const
{
	int n_x = sedid / 3;
	int n_y = sedid - n_x * 3;
	int offset = (Array_x / 2) - 1;
...
void SED_QP_MAIN::shiftcoordv4(int &xid, int &yid, VectorR3 *v4, value_t &scale) const
{
	VectorR3 center_cur;
	VectorR3 shiftvec(xid*Dx, yid*Dy, 0.0f);
...
void SED_QP_MAIN::shiftcoordv3(int &xid, int &yid, VectorR3 *v3, value_t &scale) const
{
	VectorR3 center_cur;
	VectorR3 shiftvec(xid*Dx, yid*Dy, 0.0f);
...
void SED_QP_MAIN::shiftcoordv1(int & xid, int & yid, VectorR3 & v, value_t &scale) const
{
	VectorR3 center_cur;
	VectorR3 shiftvec(xid*Dx, yid*Dy, 0.0f);
...
void SED_QP_MAIN::shiftcoordv1(int & xid, int & yid, VectorR3 & v, value_t &scale_x, value_t &scale_y, value_t &rotate_x, value_t &rotate_y) const
{
	VectorR3 center_cur;
	VectorR3 shiftvec(xid*Dx, yid*Dy, 0.0f);
...
void SED_QP_MAIN::shiftcoordv3(int &xid, int &yid, VectorR3 *v3, value_t &scale_x, value_t &scale_y, value_t &rotate_x, value_t &rotate_y) const
{
	VectorR3 center_cur;
	VectorR3 shiftvec(xid*Dx, yid*Dy, 0.0f);
...
void SED_QP_MAIN::scalecoordv1(int & xid, int & yid, VectorR3 & v, value_t &scale_x, value_t &scale_y, value_t &rotate_x, value_t &rotate_y) const
{
	value_t theta = xid * rotate_x + yid * rotate_y;
	VectorR3 v_temp = v;
	v.x = (v_temp - cen_box).x*cos(theta) - (v_temp - cen_box).y*sin(theta) + cen_box.x;
	...
	v = (v - cen_box)*(pow(scale_x, xid)*pow(scale_y, yid)) + cen_box;
}

void SED_QP_MAIN::scalecoordv3(int & xid, int & yid, VectorR3 *v3, value_t &scale_x, value_t &scale_y, value_t &rotate_x, value_t &rotate_y) const
{
	value_t theta = xid * rotate_x + yid * rotate_y;
	...
	v3[0] = (v3[0] - cen_box)*(pow(scale_x, xid)*pow(scale_y, yid)) + cen_box;
	v3[1] = (v3[1] - cen_box)*(pow(scale_x, xid)*pow(scale_y, yid)) + cen_box;
	v3[2] = (v3[2] - cen_box)*(pow(scale_x, xid)*pow(scale_y, yid)) + cen_box;
}

void SED_QP_MAIN::scalecoordv1(int & xid, int & yid, VectorR3 & v, value_t &scale) const
{
	v = (v - cen_box)*pow(scale, xid) + cen_box;
}

void SED_QP_MAIN::scalecoordv3(int & xid, int & yid, VectorR3 *v3, value_t &scale) const
{
...
void SED_QP_MAIN::shiftbedgecoordv4(int &bedge, VectorR3 *v4, value_t &scale) const
{
	if (bedge == 1)
	{
		VectorR3 shiftvec(Dx, 0.0f, 0.0f);
...
void SED_QP_MAIN::shiftbedgecoordv3(int &bedge, VectorR3 *v3, value_t &scale) const
{
	if (bedge == 1)
	{
		VectorR3 shiftvec(Dx, 0.0f, 0.0f);
...
void SED_QP_MAIN::shiftbedgecoordv1(int & bedge, VectorR3 & v, value_t &scale) const
{
	if (bedge == 1)
	{
		VectorR3 shiftvec(Dx, 0.0f, 0.0f);
...
int SED_QP_MAIN::ArrayToSED(int &_x, int &_y) const
{
	if (_x == 0)
	{
		if (_y == 0)
...
void SED_QP_MAIN::getBoundaryNum(int &_sedid, int &_b1, int &_b2) const
{
