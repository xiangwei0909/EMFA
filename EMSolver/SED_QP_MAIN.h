...
namespace mom {
	...
	protected:
		...
		CVector		REDVKernel(int &_x, int &_y, int &_id);
		void		REDVKernel_rad(int &_X, int &_y, int &_id);
		void		fillSVDI();
		void		CalculateFarCurModes();
		void		CalculateMainCur();
		void		constructI();
	...
	protected:
		...
		void		REDZ_QP_Kernel(int begin, int end);
		void		fillREDV_MP();
	...
	protected:
		...
		void		shiftcoordv1(int &sedid, VectorR3 &v, value_t &scale_x, value_t &scale_y, value_t &rotate_x, value_t &rotate_y) const;
		void		shiftcoordv3(int &sedid, VectorR3 *v3, value_t &scale_x, value_t &scale_y, value_t &rotate_x, value_t &rotate_y) const;
		...
		void		shiftcoordv3(int &xid, int &yid, VectorR3 *v3, value_t &scale_x, value_t &scale_y, value_t &rotate_x, value_t &rotate_y) const;
		void		shiftcoordv1(int &xid, int &yid, VectorR3 &v, value_t &scale_x, value_t &scale_y, value_t &rotate_x, value_t &rotate_y) const;
		...
		void		scalecoordv1(int &xid, int &yid, VectorR3 &v, value_t &scale_x, value_t &scale_y, value_t &rotate_x, value_t &rotate_y) const;
		void		scalecoordv3(int &xid, int &yid, VectorR3 *v3, value_t &scale_x, value_t &scale_y, value_t &rotate_x, value_t &rotate_y) const;
	...
	protected:
		...
		value_t		far_threshold;
		value_t		main_cur_threshold;
		...
		MatArrayEi	I_Jcm_eigen;
		MatArrayEi	I_far_eigen;
		std::vector<Qc3_mat>	DM_svd_eigen;
		Qcx_vec		I_red;
		CVector		I_red_eigen;
		Qcx_vec     V;
		Qcx_mat		V_sed;
		...
		std::vector<int> Task_assign1, Task_assign2, Task_assign3, Task_assignQP;
		component::ThreadPool pool_;
		std::vector<std::pair<int, int>> SED_task1, SED_task2, SED_taskQP;
		...
		std::vector<std::set<int>> MainRwg;
	};

	inline VectorR3 SED_QP_MAIN::centerv4(VectorR3 *v4)
	{
		return (v4[0] + v4[1] + v4[2] + v4[3]) / 4.0f;
	}

	inline VectorR3 SED_QP_MAIN::centerv3(VectorR3 *v3)
	{
